...
const JobDetails = () => {
    ...
    const { data, isLoading, refetch, error } = useFetch('job-details', { job_id: params.id })
    return (
        <SafeAreaView style={{ flex: 1, backgroundColor: COLORS.lightWhite }}>
            <Stack.Screen
                options={{
                    headerStyle: { backgroundColor: COLORS.lightWhite }, headerShadowVisible: false, headerBackVisible: false, headerLeft: () => (
                        <ScreenHeaderBtn
                            iconUrl={icons.left}
                            dimension={"60%"}
            ...
            <>
                ...
                <JobFooter url={data[0]?.job_google_link ?? 'https://careers.google.com/jobs/jobs/results'} />
            </>
        </SafeAreaView>
    )
}
